...
public class GameController : MonoBehaviour
{
    ...
    [SerializeField] private TMP_Text aplleCurrent;
    ...
    private int countEatAplle;
    private int TargetXp = 8;
    private int level;
    ...
    {
        time += Time.deltaTime;
        aplleCurrent.text = countEatAplle.ToString();
    ...
    private void CheckAppleEat() => countEatAplle++;

    private void SpawnApple() 
     {
        appleInGame = Instantiate(applePrefab, GetRandomSpawnPosition(), Quaternion.identity);
        appleInGame.GetComponent<Apple>().AppleDestroyedEvent.AddListener(CheckAppleEat);
     }
    ...
    public void ExperienceController()
    {
        ...
        XpProgressBar.fillAmount = (float)countEatAplle / TargetXp; // Convertendo para float
        if (countEatAplle >= TargetXp)
        {
            level++;
            TargetXp += 10;
        }
